...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.device = device
        self.embeddings = nn.Parameter(torch.tensor(embeddings)).to(device)
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.index_select(self.embeddings, 0, w.flatten().to(self.device)).view(batch_size, -1)
        assert x.shape == (int(batch_size), int(n_features * self.embed_size))
    ...
    def forward(self, w):
        ...
        h = F.relu(w @ self.embed_to_hidden_weight + self.embed_to_hidden_bias)
        h_d = self.dropout(h)
